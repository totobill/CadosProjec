<?php
class model_ecole extends abstract_model{
	
	protected $sClassRow='row_ecole';
	
	protected $sTable='ecole';
	protected $sConfig='pdoMysqlExple';
	
	protected $tId=array('id_ecole');
	...
	public function findById($uId){
		return $this->findOne('SELECT * FROM '.$this->sTable.' WHERE id_ecole=?',$uId );
	}
	...
	public function getSelect(){
		...
		if($tab){
		foreach($tab as $oRow){
			$tSelect[ $oRow->id_ecole ]=$oRow->nom;
		}
		}
		return $tSelect;
	}
...
class row_ecole extends abstract_row{
	
	protected $sClassModel='model_ecole';
