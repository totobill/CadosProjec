<?php
class model_adresse extends abstract_model{
	
	protected $sClassRow='row_adresse';
	
	protected $sTable='adresse';
	protected $sConfig='pdoMysqlExple';
	
	protected $tId=array('id_adresse');
	...
	public function findById($uId){
		return $this->findOne('SELECT * FROM '.$this->sTable.' WHERE id_adresse=?',$uId );
	}
	...
	public function getSelect(){
		...
		if($tab){
		foreach($tab as $oRow){
			$tSelect[ $oRow->id_adresse ]=$oRow->rue;
		}
		}
		return $tSelect;
	}
...
class row_adresse extends abstract_row{
	
	protected $sClassModel='model_adresse';
